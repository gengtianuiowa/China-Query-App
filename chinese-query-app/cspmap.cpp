            ...
            if (myfile.Database.GEOLayer[i].GEOObject->Dimension==1)//判断是否为线
            {
                ...
                for(int m=0;m<myfile.PropertyCapacity;m++)
                {
                    if(myfile.Database.GEOLayer[i].LayerName==myfile.Properties[m].LayerType)
                    {
                        ...
                        for(int m=0;m<myfile.Database.GEOLayer[i].GEOObject[j].PointCapacity;m++)
                        {
                ...
                for(int m=0;m<myfile.PropertyCapacity;m++)
                {
                     if(myfile.Database.GEOLayer[i].LayerName==myfile.Properties[m].LayerType)
                    {
                        QBrush brush(QColor(myfile.Properties[m].PlaneColor.Red,
                                     ...
                                     myfile.Properties[m].PlaneColor.Blue),Qt::Dense4Pattern);
                        for(int p=0;p<myfile.Database.GEOLayer[i].GEOObject[j].PointCapacity;p++)
                        {
...
//        temp_x.setNum(drawdata.cityFromindex.Position.X);//设置标签显示的坐标
//        temp_y.setNum(drawdata.cityFromindex.Position.Y);
//        temp_x.setNum(poschange.x());//设置标签显示的坐标
//        temp_y.setNum(poschange.y());
//        cursorAppend.append("鼠标坐标为：\n");//在小标签中添加内容
//        cursorAppend.append(" X ");
//        cursorAppend.append(temp_x);
//        cursorAppend.append(" Y ");
//        cursorAppend.append(temp_y);
//        temp_x.setNum(cursorx);
//        temp_y.setNum(cursory);
//        temp_x2.setNum(poschange.x());
//        temp_y2.setNum(poschange.y());
//        cursorAppend.append(" X ");
//        cursorAppend.append(temp_x+" ");cursorAppend.append(temp_x2);
//        cursorAppend.append(" Y ");
//        cursorAppend.append(temp_y+" ");cursorAppend.append(temp_y2);
